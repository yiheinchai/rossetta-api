...
class RossettaMiddleware:
    ...
    async def handle_http(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        # Pass through OPTIONS requests for CORS
        if method == "OPTIONS":
            await self.app(scope, receive, send)
            return
        ...
        try:
            ...
            if client_info and len(client_info) > 0:
                ...
                await self._send_json_response(
                    send, {"error": "Unable to identify client session"}, 401
                )
                return
            ...
            if abs(current_time - timestamp) > (self.max_timestamp_drift * 1000):
                await self._send_json_response(
                    send, {"error": "Request timestamp out of allowed range"}, 400
                )
                return
            ...
            if not nonce or not self.session_manager.validate_nonce(nonce):
                await self._send_json_response(
                    send, {"error": "Invalid or duplicate nonce"}, 400
                )
                return
    ...
    async def _handle_handshake_asgi(
        ...
        try:
            ...
            if not client_public_key_b64:
                await self._send_json_response(
                    send, {"error": "Missing client_public_key"}, 400
                )
                return
            ...
            if client_info and len(client_info) > 0:
                ...
                await self._send_json_response(
                    send, {"error": "Unable to identify client for session"}, 400
                )
                return
            ...
            await self._send_json_response(
                send, {"server_public_key": server_public_key_b64}, 200
            )

        except Exception as e:
            await self._send_json_response(
                send, {"error": f"Handshake error: {str(e)}"}, 400
            )

    async def _send_json_response(
        self, send: Send, content: dict, status: int = 200
    ) -> None:
        """Send JSON response"""
